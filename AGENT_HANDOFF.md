...
- **HEAD:** see `git log -1` · **Updated:** 2026-06-07 · **Active focus:** Leaderboard wager-weights card on /security shipped (build-gated, same "awaiting backend deploy" degradation as the wager-requirement cards). Prior: withdrawal wager-requirement admin UI **PARTIAL** (`c96a3075`, `b4096d61`, `dc6e05c3`) — tsc + lint + `npm run build` exit 0; live logged-in render NOT exercised (no local `.env` / DB / `SESSION_SECRET` to mint a session) and real backend success path blocked until the backend branch deploys
