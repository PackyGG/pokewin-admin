...
import { hubCodesColumns } from "./hub-codes-columns";
...
function CodeMobileCard({ code }: { code: CodeListItem }) {
  ...
  return (
    <MobileCard
      onClick={() =>
        router.push(`/creator-hub/creators/${code.ownerUserId}`)
      }
      leading={
...
export function HubCodesDataTable({ data }: { data: CodeListItem[] }) {
  const table = useReactTable({
    data,
    columns: hubCodesColumns,
          ...
          <TableBody>
            {table.getRowModel().rows.length ? (
              ...
              <TableRow className="hover:bg-transparent">
                <TableCell colSpan={hubCodesColumns.length} className="p-0">
                  <EmptyState
                    icon={Tag}
                    title="No codes found"
                    description="No affiliate codes match the current search. Try a different code or username."
