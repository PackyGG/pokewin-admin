...
/**
 * Segment-level error boundary for `/creator-hub/deal-tracker`.
 *
 * Renders INSIDE the hub shell (the layout above survives), so an
 * uncaught throw from the timeline or a deal query no longer
 ...
 */
export default function CreatorHubDealTrackerError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[creator-hub/deal-tracker] error boundary caught:", error);
  }, [error]);
    ...
    <div className="space-y-5 motion-safe:animate-in motion-safe:fade-in motion-safe:duration-300">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-semibold leading-tight tracking-tight">
              Couldn&apos;t load Deal Tracker
            </h1>
            <p className="mt-1 text-sm text-muted-foreground">
              A deal-tracker query failed before the page could render.
      ...
      <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-4">
        <p className="text-xs text-muted-foreground">
          One of the deal queries (deal windows, leaderboard milestones,
          the timeline) threw or the backend API was unreachable. If
