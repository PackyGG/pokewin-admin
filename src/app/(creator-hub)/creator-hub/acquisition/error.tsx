...
/**
 * Segment-level error boundary for `/creator-hub/acquisition`.
 *
 * Renders INSIDE the hub shell (the layout above survives), so an
 * uncaught throw from the KPI strip, charts, or funnel queries no longer
 ...
 */
export default function CreatorHubAcquisitionError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[creator-hub/acquisition] error boundary caught:", error);
  }, [error]);
    ...
    <div className="space-y-5 motion-safe:animate-in motion-safe:fade-in motion-safe:duration-300">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-semibold leading-tight tracking-tight">
              Couldn&apos;t load Acquisition
            </h1>
            <p className="mt-1 text-sm text-muted-foreground">
              An acquisition query failed before the page could render.
      ...
      <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-4">
        <p className="text-xs text-muted-foreground">
          One of the acquisition queries (signups/FTDs, daily trends, the
          conversion funnel) threw or the backend API was unreachable. If
          &quot;Try again&quot; keeps failing the issue is upstream — the
          server logs carry the full stack under the digest above.
        </p>
      </div>
