...
const dealFormSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (
      data.max_sponsorship_per_stream_usd < data.max_sponsored_battle_usd
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
...
function buildCreateDefaults(): FormState {
 ...
 * `createCreatorDeal` server action from the admin creators module;
 * validation mirrors the backend zod schema client-side.
 */
