...
export function PnlDealControls(props: {
  userId: string;
  current: PnlDealResponse | null;
  previous: PnlDealResponse[];
}) {
  const [historyOpen, setHistoryOpen] = useState(false);
  const [cancelOpen, setCancelOpen] = useState(false);
  const [reason, setReason] = useState("");
  const [pending, startTransition] = useTransition();
  const cancellable = props.current?.status === "scheduled" || props.current?.status === "active";
  return (
    <div className="flex items-center gap-2">
            ...
            {[...props.previous].sort((a, b) => b.frame_end_utc.localeCompare(a.frame_end_utc)).map((deal) => (
              <div key={deal.id} className="rounded-lg border p-3 text-xs">
                ...
                <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">
                  ...
                  <span>Payout <b>{deal.creator_share_usd == null ? "—" : formatCurrency(n(deal.creator_share_usd))}</b></span>
                </div>
      ...
      <Dialog open={cancelOpen} onOpenChange={(open) => !pending && setCancelOpen(open)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>Cancel PnL deal</DialogTitle><DialogDescription>This permanently cancels the scheduled or active frame. It cannot be resumed.</DialogDescription></DialogHeader>
