...
/**
 * Route-level error boundary for /my-profile.
 *
 * The my-profile view reads the signed-in admin's own account + activity
 * data. A failed query or a stale column would otherwise bubble to the
 * umbrella `(admin)/error.tsx`; this boundary scopes it to /my-profile.
...
export default function MyProfileError({
  ...
  useEffect(() => {
    reportWebappError({
      ...
      boundary: "(admin)/my-profile",
      error,
      digest: error.digest,
    });
    console.error("[my-profile] page error boundary caught:", error);
  }, [error]);
    ...
    <div className="space-y-5 motion-safe:animate-in motion-safe:fade-in motion-safe:duration-300">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              The profile query failed while rendering. The error was logged
              — your account is unchanged, just temporarily unreadable from
              this view.
              {error.digest && (
      ...
      <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-4">
        <p className="text-xs text-muted-foreground">
          Only the read path failed — your account data is unchanged. A
