...
/**
 * Route-level error boundary for /changelogs.
 *
 * The changelogs view reads published changelog entries. A failed query
 * or a stale column would otherwise bubble to the umbrella
 * `(admin)/error.tsx`; this boundary scopes it to /changelogs. The reset
 ...
 */
export default function ChangelogsError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[changelogs] page error boundary caught:", error);
  }, [error]);
    ...
    <div className="space-y-5 motion-safe:animate-in motion-safe:fade-in motion-safe:duration-300">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-semibold leading-tight tracking-tight">
              Couldn&apos;t load changelogs
            </h1>
            <p className="mt-1 text-sm text-muted-foreground">
              The changelogs query failed while rendering. The error was
              logged — no changelog entry was modified by this error.
              {error.digest && (
      ...
      <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-4">
        <p className="text-xs text-muted-foreground">
          Only the read path failed — published changelog entries are
