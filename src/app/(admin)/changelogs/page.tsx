    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <KpiTile
          ...
          icon={Sparkles}
          accent="emerald"
          // Auto-generated commit cards now come from the GitHub REST
          // API at request time (cached 60s) when `GITHUB_TOKEN` is set
          // in the Vercel env. Without the token the page falls back to
          // the build-time JSON at `src/lib/changelog/recent-pushes.json`
          // (regenerated by `scripts/generate-changelog.mjs` as the
          // `prebuild` step). The Info chip surfaces this so a stale
          // tile during fallback mode is explicable instead of looking
          // like a bug.
          action={
            <span
              ...
              title="Reflects live commit history when the GITHUB_TOKEN env var is set (refreshed every 60s). Falls back to the build-time snapshot otherwise — in which case commits pushed after the last Vercel deploy only appear after the next build."
              aria-label="Last published is live when GITHUB_TOKEN is set, otherwise from the last deploy"
            >
