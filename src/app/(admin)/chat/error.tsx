 ...
 *
 * The chat view reads message history / live state. A failed query or a
 * stale column would otherwise bubble to the umbrella `(admin)/error.tsx`;
 * this boundary scopes it to /chat. The reset path re-runs the server
...
export default function ChatError({
  ...
  useEffect(() => {
    console.error("[chat] page error boundary caught:", error);
  }, [error]);
    ...
    <div className="space-y-5 motion-safe:animate-in motion-safe:fade-in motion-safe:duration-300">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              The chat query failed while rendering. The error was logged —
              no message was sent, edited, or deleted as a result of this
              error.
              {error.digest && (
      ...
      <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-4">
        <p className="text-xs text-muted-foreground">
          Only the read path failed — message history is unchanged. A
