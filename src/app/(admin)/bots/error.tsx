...
/**
 * Route-level error boundary for /bots.
 *
 * The bots view reads bot-account state and activity. A failed query or
 * a stale column would otherwise bubble to the umbrella
 * `(admin)/error.tsx`; this boundary scopes it to /bots. The reset path
...
export default function BotsError({
  ...
  useEffect(() => {
    console.error("[bots] page error boundary caught:", error);
  }, [error]);
    ...
    <div className="space-y-5 motion-safe:animate-in motion-safe:fade-in motion-safe:duration-300">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              The bots query failed while rendering. The error was logged —
              no bot account was modified by this error.
              {error.digest && (
      ...
      <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-4">
        <p className="text-xs text-muted-foreground">
          Only the read path failed — bot accounts and their state are
