...
import { revealBattlePassword } from "../actions";

/**
 * Inline reveal for the plaintext password an owner set on a private
 * battle. Renders inside the Details panel of /battles/[id]. The plain
 * value is never embedded in the page payload — it's fetched on demand
 * via the revealBattlePassword server action, which audit-logs the view
 * (`battle_password_viewed`).
 *
 * Gated to role `admin` by both the parent page (conditional render) and
 * the server action itself (defence-in-depth — the action calls
 * requireRole(["admin"]) so a non-admin who got the action endpoint can't
 * bypass the UI check).
 */
