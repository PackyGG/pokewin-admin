...
/**
 * Subtle "Updated {relative}" data-freshness chip shown in the dashboard
 * hero's action slot. This is a freshness indicator, NOT a performance
 * timer — it tells the operator how stale the KPI numbers are between the
 * page's 60s server refreshes.
 *
 * `generatedAt` is the ISO timestamp the stats were produced at; the
 * relative label is recomputed on a 30s tick so "Updated 2 minutes ago"
 * stays honest between refreshes — without that tick the label would freeze
 * at "less than a minute ago" until the next router.refresh().
 *
...
export function LoadTimeIndicator({
  ...
  return (
    <span
      ...
      title={`KPI data generated ${relative}`}
    >
