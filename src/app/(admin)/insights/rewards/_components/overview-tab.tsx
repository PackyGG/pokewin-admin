...
import {
  getCreatorWithdrawalsSummary,
  type CreatorWithdrawalsSummary,
} from "@/lib/queries/insights-rewards/creator-withdrawals";
...
export async function OverviewTab({
  ...
  const [summaryRes, spendRes, creatorWdRes] = await Promise.all([
    safeQuery(
    ...
    // row, never the whole tab. Mirrors the dashboard's creatorWithdrawals
    // source (card_withdrawal_requests, role = 'creator').
    safeQuery(
  ...
  const creatorWd = creatorWdRes.error ? null : creatorWdRes.data;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Dedicated row — Total Creator Withdrawals (house cost, rose)
          and Wagering-Users P&L (the GGR the reward spend on this page
          is meant to drive, signed emerald/rose). Both keyed on the
          same window as the strip above. */}
      <FadeIn>
        <CreatorWdAndWagerPnlRow
          creatorWd={creatorWd}
          ggr={summary.ggr}
          period={period}
        />
...
// ─── Creator WD + Wagering-Users P&L row ────────────────────────────

/**
 * Two stat boxes surfaced together right under the headline strip:
 *
 *   1. Total Creator Withdrawals — sum + count of completed/shipped
 *      card_withdrawal_requests made by role = 'creator' users in the
 *      window (mirrors the dashboard's creatorWithdrawals source). A
 *      creator cashing out is money leaving the house → House-POV rose.
 *
 *   2. Wagering-Users P&L — the canonical GGR for the window from
 *      `summary.ggr` (wagers − payouts, staff excluded), the same number
 *      the strip's "GGR (window)" tile and /ggr show. Frames the house
 *      P&L generated by the real wagering customer base the reward spend
 *      on this page is meant to drive. Positive = house profit → emerald;
 *      negative = house loss → rose.
 *
 * `creatorWd === null` → that box failed its own safeQuery; render a
 * TileErrorFallback in its slot without taking down the P&L box.
 */
function CreatorWdAndWagerPnlRow({
  creatorWd,
  ggr,
  period,
}: {
  creatorWd: CreatorWithdrawalsSummary | null;
  ggr: number;
  period: InsightsRewardsPeriod;
}) {
  const label = insightsRewardsPeriodLabel(period);
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      {creatorWd === null ? (
        <TileErrorFallback
          label="Total creator withdrawals"
          hint="The creator-withdrawal query failed. Server logs hold the digest."
          size="panel"
        />
      ) : (
        <StatPanel
          title="Total creator withdrawals"
          icon={Banknote}
          accent="rose"
        >
          <p className="text-2xl font-bold leading-tight tracking-tight tabular-nums text-rose-600 dark:text-rose-400 sm:text-3xl">
            <AnimatedNumber
              value={creatorWd.total}
              format="currency"
              duration={700}
            />
          </p>
          <p className="mt-0.5 text-xs text-muted-foreground">
            {formatNumber(creatorWd.count)}{" "}
            {creatorWd.count === 1 ? "withdrawal" : "withdrawals"} · creators
            cashing out · {label}
          </p>
        </StatPanel>
      )}
      ...
      >
        ...
        <p className="mt-0.5 text-xs text-muted-foreground">
          GGR · real wagering customers · {label}
        </p>
      </StatPanel>
    </div>
  );
