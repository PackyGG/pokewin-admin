/**
 * recommend.ts — pure, rule-based scenario summarizer.
 *
 * No ML, no heuristics beyond argmax/argmin over the supplied result set.
 * Returns up to three badges:
 ...
 * concrete numbers, so the UI just renders a Badge + text (no formatting
 * logic lives in the component). Detail strings use the shared formatters
 * re-exported from `edge-calc/math.ts` so money/percent read identically to
