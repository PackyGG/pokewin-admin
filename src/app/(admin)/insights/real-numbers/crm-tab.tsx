...
/**
 * Player CRM body — lifecycle + VIP value-tier segmentation, dormant-whale
 * win-back targets and top-value players. Folded out of the former standalone
 * /crm page (which now 308-redirects to /insights/real-numbers?tab=crm) so the
 * segmentation lives as a tab on the owner-only Insights Overview. The hero is
 * supplied by the page; this component is only the streamed data body.
 *
 * Read path is unchanged: `getCrmSnapshot()` (resolveAdminRead("crm_snapshot"),
 * real customers only, borrow-corrected, last 365 days, House POV). Streamed
 * behind its own `<Suspense>` boundary on the page.
 */
