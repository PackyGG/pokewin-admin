...
/**
 * Route-level error boundary for /gift-cards.
 *
 * The gift-cards view lists redemption codes and their balance impact. A
 * failed query or a stale Prisma column would otherwise bubble to the
 * umbrella `(admin)/error.tsx`; this boundary scopes it to /gift-cards
 * with a calm "the READ failed, the codes are fine" message so admins
 * don't assume a redemption mis-fired. The reset path re-runs the server
...
export default function GiftCardsError({
  ...
  useEffect(() => {
    console.error("[gift-cards] page error boundary caught:", error);
  }, [error]);
    ...
    <div className="space-y-5 motion-safe:animate-in motion-safe:fade-in motion-safe:duration-300">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              The gift-cards query failed while rendering. The error was
              logged — no gift card was issued, redeemed, or revoked as a
      ...
      <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-4">
        <p className="text-xs text-muted-foreground">
          Only the read path failed — gift-card balances and redemption
