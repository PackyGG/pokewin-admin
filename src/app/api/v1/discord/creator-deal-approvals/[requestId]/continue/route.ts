...
export const POST = withApiKey<{ requestId: string }>(
  ...
  async (request, context) => {
    ...
    if (!params.success || !body.success) return apiError(400, "invalid_request", "Invalid creator approval continuation.");
    try {
      return await respondToCreatorDealApproval({ requestId: params.data.requestId, ...body.data, action: "continue" });
    } catch (error) {
