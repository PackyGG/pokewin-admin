...
import { prepareCreatorSetup } from "@/lib/discord-creator-setups";
import {
  DiscordIdSchema,
  jsonBody,
  rejectWrongGuild,
...
const BodySchema = z.object({
  ...
  createdByDiscordUserId: DiscordIdSchema,
  ...
  async (request) => {
    ...
    if (!parsed.success) {
      return apiError(
        400,
        "invalid_request",
        parsed.error.issues[0]?.message ?? "Invalid creator setup request.",
      );
    }
    ...
    try {
      return await prepareCreatorSetup(parsed.data);
    } catch (error) {
