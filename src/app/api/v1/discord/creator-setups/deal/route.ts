...
import { getCreatorSetupDeal } from "@/lib/discord-creator-setups";
import {
  DiscordIdSchema,
  jsonBody,
  rejectWrongGuild,
  ...
  async (request) => {
    ...
    if (!parsed.success) {
      return apiError(
        400,
        "invalid_request",
        parsed.error.issues[0]?.message ?? "Invalid creator deal request.",
      );
    }
    ...
    try {
      return await getCreatorSetupDeal(parsed.data);
    } catch (error) {
