...
import { getCreatorLastDeals } from "@/lib/discord-creator-last-deals";
import {
  DiscordIdSchema,
  jsonBody,
  rejectWrongGuild,
  ...
  async (request) => {
    ...
    if (!parsed.success) {
      return apiError(
        400,
        "invalid_request",
        parsed.error.issues[0]?.message ?? "Invalid last-deals request.",
      );
    }
    ...
    try {
      return Response.json({ data: await getCreatorLastDeals(parsed.data) });
    } catch (error) {
