...
import { getCreatorDepositSettings } from "@/lib/discord-creator-deposits";
import {
  DiscordIdSchema,
  jsonBody,
  rejectWrongGuild,
  ...
  async (request) => {
    ...
    if (!parsed.success) {
      return apiError(
        400,
        "invalid_request",
        parsed.error.issues[0]?.message ?? "Invalid creator settings request.",
      );
    }
    ...
    try {
      return await getCreatorDepositSettings(parsed.data);
    } catch (error) {
