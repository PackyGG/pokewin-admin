...
import {
  ...
  listRewardAbuseReviews,
  type RewardAbuseReview,
  type RewardAbuseStatus,
} from "@/lib/antifraud/reward-abuse";
...
export default async function RewardAbusePage({
  searchParams,
}: {
  ...
  const status = TABS.some((tab) => tab.status === params.status)
    ? (params.status as RewardAbuseStatus)
    : "pending";
      ...
      <div className="rounded-xl border bg-card p-3 sm:p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <nav
            ...
            aria-label="Reward abuse status"
          >
            {TABS.map((tab) => (
              <HostLink
                key={tab.status}
                href={`/antifraud/reward-abuse?status=${tab.status}${search ? `&q=${encodeURIComponent(search)}` : ""}`}
                className={cn(
              ...
              >
                {tab.label}{" "}
                <span className="ml-1 tabular-nums">{counts[tab.status]}</span>
              </HostLink>
            ))}
          </nav>
          <form className="flex gap-2" action="/antifraud/reward-abuse">
            ...
            <Input
              ...
              aria-label="Search reward reviews"
            />
...
function ReviewCard({ review }: { review: RewardAbuseReview }) {
  ...
  return (
    <article
      id={`review-${review.id}`}
      className="scroll-mt-20 rounded-xl border bg-card p-4 shadow-sm sm:p-5"
    >
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="space-y-2">
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              {review.score}/100 · {review.severity}
            </Badge>
            <Badge variant="secondary">{review.status}</Badge>
          </div>
          <p className="text-xs text-muted-foreground">
            Detected {formatRelative(review.lastDetectedAt)} · window ended{" "}
            {formatDateTime(review.windowEndedAt)}
          </p>
        </div>
        {review.status === "pending" ? (
          <RewardAbuseReviewActions reviewId={review.id} />
        ) : (
          <div className="max-w-md text-sm">
            <p className="font-medium">{review.reviewReason}</p>
            <p className="text-xs text-muted-foreground">
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-7">
        <Metric
          label="Rain entries"
          value={m.entries.toLocaleString()}
          note={`${m.entryDays} active days`}
        />
        <Metric
          label="Net Rain retained"
          value={usd(m.netRainUsd)}
          note={`${usd(m.rainUsd)} won · ${usd(m.rainTipsUsd)} tipped`}
        />
        <DepositsMetric
          recent={m.deposits30dUsd}
          lifetime={m.lifetimeDepositsUsd}
          rainRatio={Math.round(
            (m.deposits30dUsd / Math.max(m.netRainUsd, 0.01)) * 100,
          )}
        />
        <WithdrawalsMetric
          recent={m.withdrawn30dUsd ?? 0}
          lifetime={m.lifetimeWithdrawnUsd ?? 0}
        />
        <Metric
          label="Real-money play"
          value={usd(m.wagerUsd)}
          note={`${m.games} paid game sessions`}
        />
        <Metric
          label="Pack pattern"
          value={`${Math.round(m.packGameRatio * 100)}%`}
          note={`${m.packGames} pack sessions`}
        />
        <BonusFundingMetric
