...
export async function applyRiskyFreeBattleContainment(
  ...
  await recordRiskyFreeBattleContainmentAudit(target);
  return "locked";
}
 ...
 */
async function recordRiskyFreeBattleContainmentAudit(
  target: RiskyFreeBattleContainmentTarget,
): Promise<void> {
  await createAdminAuditEventDurable({
    ...
    metadata: {
      ...
      kind: "risky_free_battle_containment",
      // The battle and flagged-creator counts are carried inside `reason`; the
      // flagged creators themselves stay in `antifraud_signals`.
