...
export type NotifyStaffInput = {
  ...
  title: string;
  body?: string | null;
  /** In-app deep link, e.g. "/antifraud/quizzes/<id>". */
  href?: string | null;
 ...
 * Uses the antifraud host when configured, else the dashboard's own origin;
 * falls back to the bare path (still readable) when neither is set.
 */
function absoluteHref(href: string | null | undefined): string | null {
  if (!href) return null;
  if (/^https?:\/\//i.test(href)) return href;
  const base =
    process.env.NEXT_PUBLIC_ANTIFRAUD_URL ||
