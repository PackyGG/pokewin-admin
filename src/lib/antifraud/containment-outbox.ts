...
/**
 * Durable outbox for the two containment kinds whose apply step is external
 * work (a MAIN-DB write, and — for identity — a backend KYC HTTP call) that
 * must run AFTER the ADMIN ingest transaction in
 ...
 * requiring containment (a pure, DB-free decision — see
 * `fiatEligibilityContainmentTarget` / `fiatIdentityContainmentTarget`). Right
 * after that transaction commits, `ingestOne` calls `runDeferredContainment`
 * in the same request. If that throws (the exact failure mode the old code
 * masked by rolling the whole transaction back), the row is durably marked
 * `failed` with the error instead of the outcome being swallowed, and
 * `/api/cron/antifraud-containment-retry` sweeps `pending`/`failed` rows and
 * retries them — reconstructing the target purely from columns already
 * stored on the row (`kind`, `target_user_id`, `risk_score`, `payload`), so no
 * extra state needs to survive a crash beyond what the original `INSERT`
 * already committed.
 */
...
export function requiresContainmentOutbox(
  ...
  if (signal.kind === "fiat_eligibility_containment") {
...
export async function runDeferredContainment(
  ...
  `);
  try {
    let outcome: "locked" | "skipped";
    if (signal.kind === "fiat_eligibility_containment") {
      const target = fiatEligibilityContainmentTarget(signal);
      if (!target) {
        await recordContainmentOutcome(signal.signalRowId, "skipped", null);
        return "skipped";
      }
      outcome = await applyFiatEligibilityContainment(target);
    } else {
      ...
      outcome = result.locked ? "locked" : "skipped";
      // `applyFiatIdentityContainment` never throws for the KYC leg (it is
      // designed to degrade without rolling back the lock); a KYC failure is
      // logged by that module itself and does not change the outbox outcome.
    }
  ...
  const result = await adminDrizzle.execute<{
    ...
    WITH candidates AS (
      ...
      WHERE containment_outbox_status IN ('pending', 'failed')
        ...
        AND kind IN ('fiat_eligibility_containment', 'fiat_deposit_identity_containment')
