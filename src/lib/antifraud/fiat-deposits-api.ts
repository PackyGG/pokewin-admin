...
const EMPTY_DETECTION_EVIDENCE = {
  paymentIdentityHistoryStatus: "unavailable" as const,
  authorizedNetworkHistoryStatus: "unavailable" as const,
  payerEmailStatus: "unavailable" as const,
  threeDsStatus: "unavailable" as const,
  stablePaymentIdentityStatus: "unavailable" as const,
...
export const fiatDetectionEvidenceSchema = z.object({
  ...
  authorizedNetworkHistoryStatus: z.enum([
    "complete", "unavailable", "not_applicable",
  ]).default("unavailable"),
  payerEmailStatus: z.enum([
    "available", "unavailable", "not_applicable",
  ]).default("unavailable"),
  threeDsStatus: z.enum([
    "verified", "failed", "unavailable", "not_applicable",
  ]).default("unavailable"),
  stablePaymentIdentityStatus: z.enum([
    "available", "unavailable", "not_applicable",
  ]).default("unavailable"),
  checkoutEmailDiffersFromAccount: z.boolean().default(false),
  disposableCheckoutEmailDomain: z.string().nullable().default(null),
  billingCountryMismatch: z.boolean().default(false),
  checkoutEmailSharedUsers: z.number().default(0),
  whopCustomerSharedUsers: z.number().default(0),
  paymentMethodSharedUsers: z.number().default(0),
  cardSignatureSharedUsers: z.number().default(0),
  checkoutIp: z.string().nullable().default(null),
  checkoutFingerprint: z.string().nullable().optional().default(null),
  checkoutIpSharedUsers: z.number().default(0),
  checkoutDeviceSharedUsers: z.number().default(0),
  exactAmountAttempts30m: z.number().default(0),
  exactAmountDistinctUsers30m: z.number().default(0),
  exactAmountSettled7d: z.number().default(0),
  exactAmountRefunded7d: z.number().default(0),
  tipsAfterDeposit: z.number().default(0),
  tipsAfterDepositUsd: z.number().default(0),
  minutesToFirstTip: z.number().nullable().default(null),
});
...
const assessmentSchema = z.object({
  ...
  detection_evidence: fiatDetectionEvidenceSchema.optional().default(
