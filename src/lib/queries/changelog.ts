...
import { getLatestCommitsFromGitHub } from "@/lib/changelog/github";
import {
 ...
 *      `getLatestCommitsFromGitHub()` (cached 60s by `cachedGithubAutoEntries`
 *      below). This is the preferred path — it reflects commits pushed
 *      AFTER the last Vercel deploy, including commits on a feature
 *      branch that prod isn't deployed from. Requires `GITHUB_TOKEN`
 *      set in the Vercel project env. Optional knobs:
 ...
 *      step. Now a FALLBACK only — used when:
 ...
 * suppress edit / delete buttons on auto cards (they're owned by git,
 * not by the admin DB).
 */
export async function getAutoChangelogEntries(): Promise<ChangelogEntry[]> {
  // Try the live GitHub source first. The helper itself never throws
  // (it catches network / shape errors internally) — so we treat an
  // empty array as "fall back to JSON" without a try/catch here. We
  // intentionally do NOT swap to the JSON path on non-empty-but-short
  // results: if GitHub returns 3 commits and the JSON has 80, the 3
  // are still the freshest possible truth, which is what the user
  // asked for. (If GitHub returns zero, either the API is down or the
  // token is missing — JSON fallback is the right move.)
  const githubRaw = await cachedGithubAutoEntries();
  if (githubRaw.length > 0) {
    return mapRawCommitsToEntries(githubRaw);
  }

  // Fallback: the build-time JSON dump. Same parsing as before — kept
  // lenient so an older script version (missing body / filesChanged)
  // still surfaces cards.
 ...
 * coherent.
 */
const cachedGithubAutoEntries = unstable_cache(
  () => getLatestCommitsFromGitHub(),
  ["changelog-github-auto-entries-v1"],
  { revalidate: 60, tags: ["changelog"] },
);
