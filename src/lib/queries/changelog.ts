...
export async function getAutoChangelogEntries(): Promise<ChangelogEntry[]> {
  ...
  for (const item of raw) {
    ...
    // filesChanged comes from `git log --shortstat`. Null means the
    // commit didn't touch any tracked file (rare — usually a tag-only
    // bump that git log skips) OR the JSON was generated by an older
    // script version that didn't capture the field.
    const filesChanged =
      typeof item.filesChanged === "number" && Number.isFinite(item.filesChanged)
        ? item.filesChanged
        : null;
