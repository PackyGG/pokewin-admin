...
/**
 * Accounts sharing at least one Fingerprint visitor_id or the exact signup IP
 * with `sourceUserId`.
...
export async function getFingerprintAltAccounts(
  ...
  const rows = await queryMainRows<LinkedAccountRow[]>(
    `
      WITH source_identity AS (
        SELECT NULLIF(BTRIM(signup_ip), '') AS signup_ip
          FROM "user"
         WHERE id = $1
      ), source_devices AS (
        ...
        SELECT u.id AS user_id
          FROM "user" u
          CROSS JOIN source_identity source
         WHERE source.signup_ip IS NOT NULL
           AND NULLIF(BTRIM(u.signup_ip), '') = source.signup_ip
           AND u.id <> $1
      ), linked AS (
        SELECT user_id FROM device_linked
        UNION
        SELECT user_id FROM ip_linked
      )
      SELECT u.id, u.username, u.email, u.image, u.role::text AS role,
             u.roles::text[] AS roles, u.is_banned, u.created_at::text,
             COALESCE(b.total_wagered, 0)::text AS total_wagered,
             COALESCE(device_linked.shared_device_count, 0)::int AS shared_device_count,
             (ip_linked.user_id IS NOT NULL) AS shared_ip
        FROM linked
        JOIN "user" u ON u.id = linked.user_id
        LEFT JOIN balances b ON b.user_id = u.id
        LEFT JOIN device_linked ON device_linked.user_id = u.id
        LEFT JOIN ip_linked ON ip_linked.user_id = u.id
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      totalWithdrawn: pnlByUserId.get(row.id)?.withdrawals ?? 0,
