...
async function hasLastPreclaimColumn(): Promise<boolean> {
 ...
 * The checked-in Drizzle schema does NOT carry the column, though, so the typed
 * generated schema cannot reference it. We therefore probe
...
export async function getInstantRakebackLedgerTxIds(
  ...
  if (!(await hasLastPreclaimColumn())) return new Set();
  ...
  // caller holds.
  const rows = await queryMainRows<{ ledger_tx_id: string }[]>(
    `SELECT ledger_tx_id::text AS ledger_tx_id
     FROM rakeback_claims
     WHERE ledger_tx_id = ANY($1::uuid[])
