...
async function fetchTrendSeriesPg(
  ...
  const [
    ...
    db.$queryRaw<LedgerBucketRow[]>`
      SELECT ${Prisma.raw(bucketLedger)} AS bucket,
    ...
    hasUpgrader
      ? db.$queryRaw<UpgraderBucketRow[]>`
          SELECT ${Prisma.raw(bucketUpg)} AS bucket,
    ...
    db.$queryRaw<AttributionBucketRow[]>`
      WITH customers AS (
        SELECT u.id,
               EXISTS (
      ...
      SELECT ${Prisma.raw(dashboardChartBucketExpr("lt.created_at", period))} AS bucket,
    ...
    db.$queryRaw<FtdBucketRow[]>`
      ...
      SELECT ${Prisma.raw(bucketFtd)} AS bucket,
             COUNT(*)::text AS count,
