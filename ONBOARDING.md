...
- **Fiat-threshold withdrawal review:** the game backend emits a signed high-severity `fiat_deposit_withdrawal_hold` signal when the lifetime completed-deposit threshold applies balance-to-crypto and physical-item withdrawal locks. The ingest opens or updates the ADMIN-backed Account Review case, and review surfaces label the signal “Fiat-triggered withdrawal hold.” Discord delivery is separate and best-effort so an unavailable webhook cannot fail deposit completion.
