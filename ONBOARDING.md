...
- Automatic Fiat credit has two deliberately separate backend controls. The global GET/PUT `/v1/admin/fiat-deposits/config` field is `fiat_deposit_automatic_credit_enabled`; the per-user PUT `/v1/admin/users/:userId/fiat-deposit-auto-approval` field is `fiat_deposit_auto_approval_enabled`. A true per-user value overrides a false global value, while users without the override require admin approval when the global value is false. Fraud, KYC, payment-binding, dispute/refund, amount, country, and compliance checks remain independent.
