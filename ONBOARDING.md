...
- Manual per-user Fiat deposit access is backend-owned through GET/PUT `/v1/admin/users/:userId/fiat-deposit-access`. The admin user hero exposes it as an immediate on/off switch without a confirmation dialog. The dashboard and Antifraud monitor clients use `x-admin-api-key` plus the `xbypasssecret` rate-limit bypass header; neither writes MAIN directly.
