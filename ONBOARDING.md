...
- Content → Keno → Configuration edits the live maximum bet, maximum win, and three active database-backed Keno weights: withdrawal requirement (`wager_weight_keno_bps`), leaderboard (`leaderboard_wager_weight_keno_bps`), and rakeback (`rakeback_wager_weight_keno_bps`). Security exposes those same three weights inside their destination-oriented withdrawal, leaderboard, and rakeback editors; both surfaces use the same backend actions and cache tags. The legacy `shard_wager_weight_keno_bps` key is intentionally not editable because Shards are retired site-wide.
