...
- Fraud System > Config (`/antifraud/config`) owns two deliberately separate global controls. The site-wide availability switch updates `locked_deposits_fiat` through the established application mutation and refreshes the backend site-config cache; disabling it blocks card and wallet Fiat methods for everyone, while enabling it never bypasses user, country, KYC, payment, or fraud restrictions. The automatic-credit switch uses GET/PUT `/v1/admin/fiat-deposits/config` field `fiat_deposit_automatic_credit_enabled`; the per-user Account override uses PUT `/v1/admin/users/:userId/fiat-deposit-auto-approval` field `fiat_deposit_auto_approval_enabled` and may still bypass global manual review. The retired Fiat Perks screening UI, monitor routes/services, per-user grant synchronization, and eligibility allowlist gate are absent; historical Antifraud migrations and stored evidence remain untouched.
