...
- **Fiat assessment trust credit:** `fiat-v2` subtracts 20 risk points when a fiat attempt belongs to an account at least 30 days old with at least $25 of earlier completed crypto deposits backed by settlement evidence. This credit may reduce soft payment-risk signals, but it never reduces lifecycle failures, failed 3DS, risky payment history, account/KYC restrictions, repeated failures, linked-account evidence, or post-deposit cash-out risk. Email-domain and suspicious Gmail-alias matches are applied after ordinary scoring and always force score 100, a bad verdict, the existing withdrawal lock, and a backend-owned KYC cycle. Gmail alias detection includes both heavy dot fragmentation and the confirmed long-name plus coded/numeric suffix shape; ordinary names, initials, and simple numbered addresses remain allowed. Already-required accounts keep their current cycle.
...
- **Free-battle fraud relationships:** `antifraud-monitor` continuously scans free/sponsored battle joins on the read-only MAIN mirror for creators with rejected or fraud-specific KYC, suspected-alt flags, or active Antifraud risk. It stores deduped evidence and per-creator cursors in the Antifraud DB. One qualifying battle is evidence only; two distinct qualifying battles automatically require KYC and lock crypto withdrawals plus item shipping, whether they came from the same fraud creator or different fraud creators. A signed, deduped containment event crosses into the dashboard's application-authorized MAIN mutation boundary. Dashboard delivery uses a per-event acknowledgement timestamp rather than trusting a tuple cursor alone, so a transaction that commits after a newer event cannot be skipped; the cursor is retained only as monotonic compatibility state.
...
- **Abstract signup enrichment:** every new signup also receives cached, required Abstract IP Intelligence and Email Reputation checks. IP evidence covers VPN, proxy, Tor, hosting, relay, abuse, ASN/company, and country mismatch. Email evidence covers deliverability, SMTP/MX, catch-all, disposable, username quality, address/domain risk, TLD, and domain age. Address `minimum_age` and domain `domain_age` remain distinct; only the latter can trigger the new-domain score. Only sanitized email-provider evidence is retained; sender names, raw MX hosts, and breach-domain lists are discarded. An Abstract-confirmed catch-all result is a 100-point signal that opens review, applies the reversible full-account lock, requires KYC, and locks crypto/item withdrawals through the signed containment path. That containment event commits independently before an unrelated required-provider failure enters the durable signup dead-letter, so a Fingerprint, ProxyCheck, IP, or Opportify outage cannot postpone a confirmed catch-all lock.
...
- **Automatic Fiat eligibility:** `POST /v1/fiat-eligibility/check` is a binary server-to-server checkout gate with dedicated, environment-bound dev/prod credentials and caller-IP allowlists. `fingerprint` is a fresh Fingerprint request ID linked to the requested user. The monitor always performs current Fingerprint Pro Plus and proxycheck.io checks for a valid subject, compares signup IP/device and account history, honors user/country Fiat locks and KYC, persists the decision in the Antifraud DB, and fails closed. Production uses the normal read-only MAIN mirror; dev requires its own optional read-only source URL. A Fingerprint request ID is single-use and allow decisions expire after 60 seconds.
...
- **Risky signup locations:** manager-only `/antifraud/risky-locations` stores enabled ISO country rules and a bounded 1–60 minute monitor duration in the dedicated Antifraud database. A matching signup starts a monitor even at zero risk points, uses the country-specific duration instead of the normal three-minute default, and retains a zero-point location signal explaining why the session opened. Long windows increase active-session polling in proportion to matching signup volume, so use 60-minute rules selectively.
...
- Manual per-user Fiat deposit access is backend-owned through GET/PUT `https://packy.gg/v1/admin/users/:userId/fiat-deposit-access`. The admin user hero exposes it as an immediate on/off switch without a confirmation dialog. The dashboard and Antifraud monitor clients use `x-admin-api-key` plus the `xbypasssecret` rate-limit bypass header; neither writes MAIN directly.
