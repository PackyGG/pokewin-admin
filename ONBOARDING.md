...
- **Antifraud withdrawal tracking:** `/antifraud/withdrawals` reads only the authenticated monitor API. The monitor service reconciles the selected MAIN withdrawal page against attached inventory/voucher origins and a bounded 90-day ledger window, checks account age, deposit-to-withdrawal timing, and payout-destination reuse, then stores the derived score/evidence in Antifraud `withdrawal_assessments`. MAIN remains read-only; the assessment is refreshed when staff load the list.
...
- Pack Builder inactive builds are ADMIN-only saved drafts (`pack_creation_requests.requested_active = false`) shown on `/pack-studio/builder-drafts`; saving needs no owner approval and never touches MAIN. A builder can later promote a saved draft into a live request. Only live requests appear on the owner-only **System → New Packs** page, where approval revalidates and materializes the pack in MAIN. Approval/decline is one click with no extra 2FA; direct activation remains blocked for non-owner `pack_creator` users.
- Pack Builder edge is constrained to the inclusive **10.95%–12.00%** band. The builder clamps its configured curve into that range and rejects an out-of-range target or freshly re-shaped result both when the request is queued and when the owner approves it.
...
- The dashboard Keno KPI is an active-window settled-performance view over `keno_games`, scoped to real customers (staff, creators, and the admin blacklist excluded). It reports wager, player payouts, profit (`wager - payouts`), and realized edge (`profit / wager`); it is not the configured mathematical edge shown on Odds & Chances.
