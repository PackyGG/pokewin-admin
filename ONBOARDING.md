...
- **Antifraud notification registry:** `services/antifraud-monitor/src/notification-routes.ts` is the sole route/event-family catalog for all five Discord destinations and signed dashboard ingest. Producers resolve their targets through it. Manager-only `/antifraud/webhooks` reads the dedicated authenticated `/v1/operations/notifications` status endpoint, whose public route objects contain only label, purpose, event families, and configured state. Do not duplicate the catalog in dashboard code or expose destination identifiers or credentials.
