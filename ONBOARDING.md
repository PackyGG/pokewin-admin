...
- Pack Builder creation is approval-gated: every draft or live request is stored in the ADMIN `pack_creation_requests` queue, and only an owner can materialize it in MAIN from the dedicated Packs workspace’s **System → New Packs** page. The page is owner-only in both navigation and its server route. Approval/decline is one click with no extra 2FA; direct activation remains blocked for non-owner `pack_creator` users.
