...
- **Fiat failure operations:** the repository-contained Antifraud monitor observes `fiat_deposit_intents` and `payment_webhook_events` through its read-only MAIN mirror. Failed, review, disputed, refund, stalled checkout-creation, long-pending, and failed-webhook states enter a deduplicated Antifraud-DB outbox and retry a dedicated Discord webhook. Ordinary canceled checkouts are intentionally excluded; webhook failure discovery uses a bounded rolling scan because failed webhook rows do not set `processed_at`. Mirror connection exhaustion does not block the service API or Antifraud DB from starting; the poller retries independently and `/ready` stays unavailable until source processing recovers.
