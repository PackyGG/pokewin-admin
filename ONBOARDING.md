...
- **Free-battle fraud relationships:** `antifraud-monitor` continuously scans free/sponsored battle joins on the read-only MAIN mirror for creators with rejected or fraud-specific KYC, suspected-alt flags, or active Antifraud risk. It stores deduped evidence and per-creator cursors in the Antifraud DB. One qualifying battle is evidence only; two distinct qualifying battles automatically require KYC and lock crypto withdrawals plus item shipping, whether they came from the same fraud creator or different fraud creators. A signed, deduped containment event crosses into the dashboard's application-authorized MAIN mutation boundary.
