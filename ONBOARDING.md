...
- **Fiat assessment trust credit:** `fiat-v2` subtracts 20 risk points when a fiat attempt belongs to an account at least 30 days old with at least $25 of earlier completed crypto deposits backed by settlement evidence. This credit may reduce soft payment-risk signals, but it never reduces lifecycle failures, failed 3DS, risky payment history, account/KYC restrictions, repeated failures, linked-account evidence, or post-deposit cash-out risk. Email-domain and Gmail-fragmentation matches are applied after ordinary scoring and always force score 100, a bad verdict, the existing withdrawal lock, and a backend-owned KYC cycle. Already-required accounts keep their current cycle.
