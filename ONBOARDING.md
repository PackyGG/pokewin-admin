...
- **Operator identifier blocklists:** migration 039 adds canonical IPv4/IPv6 exact and CIDR rules plus exact fingerprint rules in the Antifraud DB. Rules default permanent, may expire, and are disabled/reactivated rather than deleted; every mutation and historical review-only match is durable. New active matches lock withdrawals and open review through signed ingest, never KYC. Every staff single-account ban first permanently blocks all known signup/captured IPs and Fingerprint visitor IDs and aborts before the account write if a known identifier cannot be saved. `/users/[id]` also provides separate IP-only and fingerprint-only actions that resolve current identifiers server-side. The Profiles workspace exposes sanitized permanent assessment, provider, relationship, and blocklist evidence; MAIN status enrichment is an exact mirror read.
...
- `/antifraud/fiat-perks` is the screening and controlled-rollout workspace. Screening may target a predefined cohort, country, or explicit account IDs and persists full normalized evidence including MaxMind Factors scores, disposition, risk reasons, subscores, and warnings. Staff can filter candidates by access state, verdict/decision, country, internal or MaxMind risk range, MaxMind status/disposition, provider completeness, age, blocking reason, funding, wager, and reward exposure.
