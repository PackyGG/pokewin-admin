...
- **Fiat failure operations:** the repository-contained Antifraud monitor observes `fiat_deposit_intents` and `payment_webhook_events` through its read-only MAIN mirror. Failed, review, disputed, refund, stalled checkout-creation, long-pending, and failed-webhook states enter a deduplicated Antifraud-DB outbox and retry the fiat-operations Discord destination. Canonical high-risk and fiat-locked-account deposits fan out to both general risk and fiat operations through per-destination delivery rows, so partial failures retry independently without duplicating the successful route. Alert embeds and buttons use the live `fraud.packydash.com/fiat-deposits` workspace. Ordinary canceled checkouts are intentionally excluded; webhook failure discovery uses a bounded rolling scan because failed webhook rows do not set `processed_at`. Mirror connection exhaustion does not block the service API or Antifraud DB from starting; the poller retries independently and `/ready` stays unavailable until source processing recovers.
