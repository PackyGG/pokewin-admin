...
- **Fiat Perks screening progress:** `/antifraud/fiat-perks` persists candidate and provider-check totals after every screening batch. The monitor broadcasts started, progress, completed, and failed run frames through the durable Redis/WebSocket live transport; the dashboard consumes them through its authenticated SSE bridge and keeps polling only as a disconnected safety fallback.
...
- Pack Builder inactive builds are ADMIN-only saved drafts (`pack_creation_requests.requested_active = false`) shown in **Saved Builds** at `/pack-studio/builder-drafts`; saving needs no owner approval, may omit artwork, and never touches MAIN. Exact displayed ticket weights, colors, and animations are the saved contract. Drafts carry an optimistic `revision` guard, browser-local recovery, and durable `pack_build_draft_revisions` snapshots that restore forward as a new revision. Pack Builders see only their own saved builds, while owners/admins see the shared workspace. Saved Builds lets an authorized builder or owner add or replace the draft image. A valid image is required server-side before direct live submission, draft promotion, and final owner approval. Only live requests appear in the owner-only **System → Approval Queue**, where approval revalidates and materializes the pack in MAIN. Approval/decline is one click with no extra 2FA; direct activation remains blocked for non-owner `pack_creator` users.
- Pack Builder production math is strict: house edge must be inside the inclusive **10.95%–11.50%** band, every card must be unique with positive odds, max win must stay under the configured cap, and final persisted weights must total exactly **1,000,000 integer ticket units (100.0000%)**. Auto mode additionally requires a feasible solver result; manual mode validates the exact displayed pool directly and does not depend on the solver. Submission preview, saved-build/approval UI, queued-request validation, fresh owner approval, and a second check over the exact rows entering the transaction all fail closed.
- Mutating actions must `createAdminAuditEvent()`. 2FA-gate sensitive mutations (balance, XP, withdrawals).
- **Login supports either direct passkey or password → second factor.** `/login` can authenticate straight from a discoverable WebAuthn credential with required device user verification; no email/password or `/verify-2fa` step is needed. The password path remains available and accepts either TOTP/recovery or a passkey at `/verify-2fa`. New passkey enrollment requires discoverability plus user verification so it supports direct login; older non-discoverable credentials may need to be removed and re-added. Passkeys live in ADMIN table `admin_passkeys`; enrollment is in profile Security (`src/app/(admin)/profile/passkeys-card.tsx` + `passkey-actions.ts`). RP/origin checks live in `src/lib/webauthn.ts`, the 5-minute signed challenge cookie is `admin_webauthn_challenge`, counters and `last_used_at` retain replay evidence, and every successful passkey login uses the normal session/audit path with `method:"passkey"`.
...
- Analytics → Games → Keno → Configuration edits only the live maximum bet and maximum win. Security is the sole editor for the three active database-backed Keno weights: withdrawal requirement (`wager_weight_keno_bps`), leaderboard (`leaderboard_wager_weight_keno_bps`), and rakeback (`rakeback_wager_weight_keno_bps`). The legacy `shard_wager_weight_keno_bps` key is intentionally not editable because Shards are retired site-wide.
- The dashboard Keno KPI and Analytics → Games → Keno overview are settled-performance views over `keno_games`, scoped to real customers (staff, creators, and the admin blacklist excluded). The dashboard KPI is an exact five-minute-cached lifetime aggregate, matching Upgrader, while the Keno overview retains its operational lifetime/24h/7d breakdowns. They report wager, player payouts, profit (`wager - payouts`), and realized edge (`profit / wager`); they are not the configured mathematical edge shown on Odds & Chances. The legacy `/keno` route redirects to this integrated view.
...
- `GET /v1/keno/multipliers` returns the backend table only in development/test. The backend deliberately registers no Keno routes in production, so Analytics → Games → Keno → Odds & Chances uses the tested compile-time mirror in `src/lib/keno/payouts.ts`. Settled `keno_games` rows are evidence/drift detection only, never the source for unobserved configured multipliers.
