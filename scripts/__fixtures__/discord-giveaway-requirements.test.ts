...
test("giveaway linked-account requirements are durable and server-enforced", async () => {
  const [migration, service, createRoute, entryRoute] = await Promise.all([
  ...
  assert.match(createRoute, /entryRequirement: z\.enum\(\["none", "linked_packy_account"\]\)/);
