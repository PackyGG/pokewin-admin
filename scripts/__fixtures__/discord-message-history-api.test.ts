...
test("Discord message history is bounded, private, durable, and idempotent", async () => {
  const [route, service, migration, exclusionMigration, includeAdminsMigration, schema, scopes, endpoints] = await Promise.all([
    ...
    read("drizzle/admin/migrations/20260806_discord_message_history_include_admins.sql"),
  ...
  assert.doesNotMatch(service, /MESSAGE_LOG_EXCLUDED_USER_IDS/);
  ...
  assert.match(includeAdminsMigration, /DELETE FROM "discord_message_snapshots"/);
  assert.match(includeAdminsMigration, /"author_is_bot" IS NOT TRUE/);
  assert.match(includeAdminsMigration, /"webhook_id" IS NULL/);
  for (const adminId of [
    "660132586630414338",
    "934854938641715240",
    "188051599099297802",
  ]) assert.match(includeAdminsMigration, new RegExp(adminId));
